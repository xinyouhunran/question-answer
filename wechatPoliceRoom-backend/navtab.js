   ...
   var  element = layui.element(),
        ...
        LarryTab.prototype.tabAdd = function(data){
		    ...
		    if(tabIndex === -1){
			    ...
			    //ls add
			    //删除下拉展开的按钮
		            if($(".layui-tab-bar").length>0){
		               	console.log(6,$(".layui-tab-bar"));
		            ...
		            if(mw>ltt){
		            	/*n++;//超出次数
		            	console.log(n,ali.length-1);
		            	for(i=0;i<n;i++){
		            		lw+=$(".layui-tab-title li").eq(ali.length-1-i).width()+20;
		            	}
		            	
		            	lw = -lw+100+"px";*/
		            	$(".layui-tab-title").width(mw);
		            	lw = -(mw-ltt)+50+'px';
		            	console.log(lw);
		            	$(".layui-tab-title").animate({"left":lw},100);
		            }
			    //iframe 自适应
				...
				//监听关闭事件
				    ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
				    	/*var lw = $(".layui-tab-title").css("left");
